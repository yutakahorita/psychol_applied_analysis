...
グループBの母集団の平均値を1かつ標準偏差を1($\mu_{B} = 1, \sigma_{B} = 1$)として、それぞれのグループから100個($n=100$)のサンプルを抽出し、t検定を行った結果である。
...
p値は`r round(result$p.value, 3)`であり、有意水準5%とすると有意な差があるという結論は出ない。つまり、10人ずつ標本を抽出したこの結果からは、帰無仮説$H_{0}:\mu_{A} - \mu_{B}=0$を棄却することはできない。
...
for(delta in c(0.2, 0.5, 0.8)){
  for(alpha in seq(0.001, 0.10, 0.001)){
    result = power.t.test(n = 50, delta = delta, sd = 1, sig.level = alpha) #pwrパッケージのpower.t.testを使って検出力を求める
    type_2 = 1 - result$power #type 2 error
...
for(delta in c(0.2, 0.5, 0.8)){
  for(n in 2:100){
    result = power.t.test(n = n, delta = delta, sd = 1, sig.level = 0.05) #pwrパッケージのpower.t.testで検出力を求める
    power = result$power
...
今度は、同じ平均0、標準偏差1の正規分布に従う母集団AとBから1000個ずつ標本を抽出しグループAとBの間で平均値に差があるかをt検定で検定する。帰無仮説は$H_{0}:\mu_{A} = \mu_{B}$である。母集団の真の平均値差もゼロ（母集団AとBは全く同じ分布）なので、帰無仮説が正しいという前提である。  
