...
ggplot2::ggplot() + 
  ggplot2::geom_point(data = dat_poisson, aes(x = x, y = y)) + 
  ggplot2::geom_smooth(data = dat_poisson, aes(x = x, y = y), 
                       formula = "y ~ x",
                       method = "glm",
...
## ポアソン回帰の係数の解釈{#chap13_glmslope}

ロジスティック回帰のときと同様に、ポアソン回帰の予測変数の傾きの値も単純に「予測変数が1単位増えたときの応答変数の変化量」を意味しないことに注意が必要である。ポアソン回帰のモデルをもう一度確認すると、

$$
\log\lambda_{i}=\alpha+\beta x \\
$$
であった。つまり、予測変数の傾きは「その予測変数が1単位増えたときの、応答変数（パラメータ$\lambda$）の対数の変化量」を意味する。しかし、対数の変化量は直感的に理解しにくい。  
  
上記の式は、以下のように直すことができる。
...
\end{equation}
$$  
すなわち、予測変数が1単位増えたときに、応答変数（パラメータ$\lambda$）は$\exp(\beta)$倍変化すると解釈することができる。

予測変数の応答変数に対する効果の大きさを解釈する場合には、指数関数で傾きを変換させる必要がある。  
  
Rでは、以下のようなプログラムで求めることができる。

```{r}

result_poisson = glm(data = dat_poisson, y ~ 1 + x, family = poisson(link = "log")) #glmで求めた結果を別の名前で保存する
coef(result_poisson) #coef関数で係数の部分を取り出すことができる
exp(coef(result_poisson)) #指数関数で変換する
...
`x`の傾きのp値はかなり小さく、`x`は`y`を予測する上で有意な効果を持っていると推定される。しかし、単純にそのような結論を出すことはできない。このデータでは、分散が平均よりも過剰に大きい**過分散**が生じているためである。
...
分散が平均よりも大きいときにポアソン回帰を使うと、分散を実際よりも小さく推定してしまう恐れがある（p値を過剰に小さく判断してしまい、第一種の過誤を犯しやすくなる）。この問題は、**過分散(overdispersion)**と呼ばれる。  
